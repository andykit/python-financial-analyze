...
Data=pd.DataFrame(D)
import apriori as ap   #导入关联规则算法程序
support = 0.47 #最小支持度
confidence = 0.9 #最小置信度
ms = '--' #连接符，
outputfile = 'apriori_rules3.xls' #结果文件
ap.find_rule(Data, support, confidence, ms).to_excel(outputfile) #调用关联规则挖掘函数
